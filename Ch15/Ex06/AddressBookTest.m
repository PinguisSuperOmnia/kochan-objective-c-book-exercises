...
int main(int argc, char *argv[])
{
	...
	[card2 setName: bName andEmail: bEmail andAddress: @"9999 philly st, philadelphia, pa" andPhone: @"987-654-3210"];
	...
	NSMutableArray *results = [myBook lookup: @"ephen"];
	
	for ( AddressCard *theCard in results ) {
		[theCard print];
	}
	
	NSLog(@"removing all epehens");
	[myBook removeName: @"ephen"];
	[results release];
	
	NSLog(@"Lookup: ephen");
	results = [myBook lookup: @"ephen"];
